...
def review_predict(new_sentence):
    model = load_model('c:/python/RNN_model.h5')
    tokenizer = joblib.load('c:/python/tokenizer.h5')
    okt = Okt()
    new_sentence = okt.morphs(new_sentence, stem=True) # 토큰화
    stopwords = ['의', '가', '이', '은', '들', '는', '좀','잘', '걍', '과', '도', '를', '으로', '자', '에', '와', '한','하다']
    new_sentence = [word for word in new_sentence if not
    word in stopwords] # 불용어 제거
