...
def list(request):
    ...
    try:
        ...
        search = ""
    boardCount=Board.objects.count() #레코드 수
    try:
        start = int(request.GET['start']) #레코드 번호
    except:
        start = 0
    ...
    {"boardList": boardList, "search_option": search_option,"search":
    search, "range": range(start_page - 1, end_page), "start_page":
    start_page,"end_page": end_page, "page_list_size": page_list_size,
    "total_page": total_page, "prev_list": prev_list, "next_list":
    next_list, "links": links})
def write(request):
